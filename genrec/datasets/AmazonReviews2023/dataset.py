...
class AmazonReviews2023(AbstractDataset):
    ...
    def _timestamp_split(self, datasets):
        ...
        for user, parent_asin, history in zip(
            ...
            count+=1
            user_test.add(user)  # 收集测试集user
            item_test.add(parent_asin) # 收集测试集item
            if count > self.max_rows * len(datasets[split]): # sample 出来这么多测试集
                break
        self.log(f'[DATASET] test user {len(list(user_test))}')
        self.log(f'[DATASET] test item {len(list(item_test))}')

        for split in ['train', 'valid', 'test']:
            self.log(f'[DATASET] {split} original size {len(datasets[split])}')
            ...
            for user, parent_asin, history in zip(
                ...
                if user not in user_test:
                    continue
                items_in_history = history.split(' ')
                items_in_history.append(parent_asin) # 最后一个是target
                split_data[split]['user'].append(user)
                split_data[split]['item_seq'].append(items_in_history)
            self.log(f'[DATASET] {split} size {len(split_data[split]["user"])}')
    ...
    def _download_and_process_raw(self):
        ...
        #     self.cache_dir,
        #     f"raw/{self.config['kcore']}/{self.config['split']}_w_his"
        # )
        ...
        # import 
        # for split in ['train', 'valid', 'test']:
        #     filtered_dataset = datasets[split].filter(lambda t: len(t['history']) > 0)
        #     truncated_dataset = filtered_dataset.map(
        #         lambda t: {
        #             'history': ' '.join(t['history'].split(' ')[-self.max_item_seq_len:])
        #         },
        #     )
        #     datasets[split] = truncated_dataset
            ...
            # for split in ['train', 'valid', 'test']:
            #     out_path = os.path.join(raw_data_dir, f"{self.category}.{split}.csv")
            #     datasets[split].to_csv(out_path, index=False)
        # import ipdb; ipdb.set_trace() 
        # 
