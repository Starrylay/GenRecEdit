...
def get_file_name(config: dict, suffix: str = ''):
    ...
    # )
    if config.get("deltaW_path", None) is not None:
        deltaW_path = config["deltaW_path"]
        parent_dir = os.path.basename(os.path.dirname(deltaW_path))          # "TIGER_${category}"
        stem = os.path.splitext(os.path.basename(deltaW_path))[0]  
        deltaW_stem= parent_dir + "_"+stem
        logfilename = "Edit-{}-{}{}".format(
            deltaW_stem, config['run_local_time'], suffix
        )
...
def get_tokenizer(model_name: str):
    ...
    try:
        # import ipdb; ipdb.set_trace()
        module = importlib.import_module(f'genrec.models.{model_name}.tokenizer')
        tokenizer_class = getattr(module, f'{model_name}Tokenizer')

    except Exception as e:
        raise ValueError(f'Tokenizer for model "{model_name}" not found. Real error: {e!r}') from e
    return tokenizer_class
