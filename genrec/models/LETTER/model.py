...
class LETTER(AbstractModel):
    ...
    # TODO: LETTER模型训练上相比于TIGER多加了loss
    # 就是多除了一个温度系数
    # LETTER_TIGER
    def __init__(
        self,
        ...
        super(LETTER, self).__init__(config, dataset, tokenizer)
        ...
        self.loss_fct = torch.nn.CrossEntropyLoss(
            ignore_index=tokenizer.ignored_label, reduction='mean')
    ...
    def forward(self, batch: dict) -> torch.Tensor:
        ...
        loss = self.loss_fct(t_logits.view(-1, t_logits.size(-1)),
                             labels.view(-1))
        outputs.loss = loss
    ...
    def generate(self,
                 batch: dict,
                 n_return_sequences: int = 1) -> torch.Tensor:
        """
        ...
        if batch['split'] == 'val':
            outputs = self.beam_search(input_ids=batch['input_ids'],
                                       attention_mask=batch['attention_mask'],
                                       max_length=n_digit + 2,
                                       num_beams=self.config['val_num_beams'],
            ...
            outputs = self.beam_search(input_ids=batch['input_ids'],
                                       ...
                                       num_return_sequences=n_return_sequences,
                                       return_score=False)
        outputs = outputs[:, 1:1 + n_digit].reshape(-1, n_return_sequences,
                                                    n_digit)
        return outputs

    @torch.no_grad()
    def get_per_token_metrics(self, batch: dict, topk_list, use_gt=False):
        assert np.max(topk_list) <= self.config['num_beams']
        ...
        decoder_input_ids = torch.ones(
            (batch_size, 1), device=self.t5.device,
            dtype=torch.long) * self.t5.config.decoder_start_token_id
        # initial_decoder_input_ids = decoder_input_ids * self.t5.config.decoder_start_token_id

        encoder_outputs = self.t5.get_encoder()(input_ids=input_ids,
                                                attention_mask=attention_mask,
                                                return_dict=True)

        per_token_recall = {}
        for i in range(self.tokenizer.n_digit):
            outputs = self.t5(encoder_outputs=encoder_outputs,
                              attention_mask=attention_mask,
                              decoder_input_ids=decoder_input_ids)
            logits = outputs.logits

            _, pred_next_token = torch.topk(logits[:, -1, :],
                                            k=num_beams,
                                            dim=-1)

            pos_index = (pred_next_token == labels[:, i:i + 1]).long()

            for k in topk_list:
                per_token_recall[
                    f'Token-{i}/Recall@{k}'] = pos_index[:, :k].sum(
                        dim=-1).detach().cpu().tolist()

            if use_gt:
                next_token = labels[:, i].unsqueeze(1)
            else:
                # NOTE: 直接greedy-search
                next_token = pred_next_token[:, 0].unsqueeze(1)

            # decoder_input_ids = torch.cat(
            #     [initial_decoder_input_ids, next_token], dim=-1)
            decoder_input_ids = torch.cat([decoder_input_ids, next_token],
                                          dim=-1)

        return per_token_recall
    ...
    def prepare_beam_search_inputs(self, input_ids, attention_mask, batch_size,
        ...
        decoder_input_ids = torch.ones((batch_size * num_beams, 1),
        ...
        beam_scores[:,
                    1:] = -1e9  # Set a low score for all but the first beam to ensure the first beam is selected initially
        initial_beam_scores = beam_scores.view((batch_size * num_beams, ))

        beam_idx_offset = torch.arange(
            batch_size,
            device=self.t5.device).repeat_interleave(num_beams) * num_beams
    ...
    def beam_search_step(self, logits, decoder_input_ids, beam_scores,
        ...
        next_token_scores = torch.log_softmax(
            next_token_logits,
            dim=-1)  # Calculate log softmax over the last dimension
