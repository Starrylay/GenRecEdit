...
class TIGER(AbstractModel):
    ...
    def __init__(
        ...
        if self.deltaW_path is not None:
    ...
    def beam_search(
        ...
        *** This implementation does not include stopping conditions based on end-of-sequence (EOS) tokens. Instead, the
        ...
        while decoder_input_ids.shape[1] < max_length:
            ...
            if decoder_input_ids.shape[1] <= 4:
                layer_idx = decoder_input_ids.shape[1] -1
                layer_name = f'decoder.block.{layer_idx}.layer.2.DenseReluDense.wo'
                param_layer = f'decoder.block.{layer_idx}.layer.2.DenseReluDense.wo.weight'
                with torch.no_grad():
                    current_param = nethook.get_parameter(self.t5, param_layer)
                    delta_W = all_weight_updates[param_layer].to(current_param.device)  # [d_model, d_ff]
                    current_param.add_(delta_W)
                    # print(f"✓ Updated {param_layer}")
                    # 存储 all_weight_updates
            else:
                layer_name = None
        # ===============================END edit==========================================
            with torch.no_grad():
            ...
            if layer_name is not None:
                with torch.no_grad(): 
                    init_param = all_weight_init[param_layer].to(current_param.device, dtype=current_param.dtype)
