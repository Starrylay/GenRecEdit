...
class TIGERTokenizer(AbstractTokenizer):
    ...
    def __init__(self, config: dict, dataset: AbstractDataset):
        ...
        self.id2item = dataset.id_mapping['id2item'] # id 0 不是item; id 1开始对应item
    ...
    def _init_tokenizer(self, dataset: AbstractDataset):
        ...
        # import ipdb; ipdb.set_trace()
        # print(sem_ids_path, item2sem_ids["B00JVUXQW4"],)
    ...
    def _token_single_user(self, user: str) -> int:
        ...
        return self.base_user_token + user_id % self.n_user_tokens # TIGER 似乎设置了n_user_tokens为1 这样每个用户的id都一样
    ...
    def tokenize_function(self, example: dict, split: str) -> dict:
        ...
        """
        if split == 'train':
            n_return_examples = len(example['item_seq'][0]) - 1    # 窗口实现训练集
    ...
    def tokenize(self, datasets: dict) -> dict:
        ...
        for split in datasets:
            tokenized_datasets[split] = datasets[split].map(
                lambda t: self.tokenize_function(t, split),  # 由于datasets[split]是Dataset对象 所以t是一个example {'user': [], 'item_seq': []}
                batched=True,
                batch_size=1,
